...
export function HeroSection({ settings }: HeroSectionProps) {
  ...
  const heroName = settings?.hero_name || person.name;
  const heroHeadline =
    settings
      ? lang === "en"
        ? settings.hero_headline_en
        : settings.hero_headline_id
  ...
  const ctaText =
    settings
      ? lang === "en"
        ? settings.hero_cta_text_en
        : settings.hero_cta_text_id
      : "About Me";
  const ctaLink = settings?.hero_cta_link || about.path;
  const avatarSrc = settings?.avatar || person.avatar;
